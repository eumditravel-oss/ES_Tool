...
const DETAIL_FIELDS = [
  ...
  "machine","misc","mine","manufacture","utility",
  "agri","g1","g2","g3","g4","g5",
  "planProgress","actProgress","psAmount","deduction","note"
];
...
function itemExpenseTotal(item) {
  return toNumber(item.machine) + toNumber(item.misc);
}

function calcAttach6Values(item) {
  const amountA = Math.max(toNumber(item.total), 0);
  const planB = Math.max(toNumber(item.planProgress), 0);
  const actC = Math.max(toNumber(item.actProgress), 0);
  const fastD = Math.max(planB, actC);
  const psE = Math.max(toNumber(item.psAmount), 0);
  const classifiedF = Math.max(amountA - fastD - psE, 0);
  const deductionG = Math.max(toNumber(item.deduction), 0);
  const excludedH = Math.max(fastD, deductionG) + psE;
  const applicableI = Math.max(amountA - excludedH, 0);
...
function renderWorkGrid() {
  ...
  if (!items.length) {
    body.innerHTML = `<tr><td colspan="25">조건에 맞는 항목이 없습니다.</td></tr>`;
    return;
  }
  ...
  body.innerHTML = items.map((item) => `
    <tr class="${item.id === state.selectedItemId ? "active-row" : ""}" data-row-id="${item.id}">
      ...
      <td><input type="number" data-field="machine" data-id="${item.id}" value="${item.machine ?? 0}"></td>
      ...
      <td><input type="number" data-field="planProgress" data-id="${item.id}" value="${item.planProgress ?? 0}"></td>
      <td><input type="number" data-field="actProgress" data-id="${item.id}" value="${item.actProgress ?? 0}"></td>
      <td><input type="number" data-field="psAmount" data-id="${item.id}" value="${item.psAmount ?? 0}"></td>
      <td><input type="number" data-field="deduction" data-id="${item.id}" value="${item.deduction ?? 0}"></td>
  ...
  body.querySelectorAll("[data-field][data-id]").forEach((input) => {
    ...
    const numberFields = [
      "total","labor","machine","misc","mine",
      "manufacture","utility","agri","g1","g2","g3","g4","g5",
      "planProgress","actProgress","psAmount","deduction"
    ];
...
function renderMiniGrid() {
  ...
  body.innerHTML = items.slice(0, 12).map((item) => {
    const attach6 = calcAttach6Values(item);
    return `
    <tr>
      <td>${item.code || "-"}</td>
      <td>${item.name || "-"}</td>
      <td>${formatNumber(item.total)}</td>
      <td>${formatNumber(attach6.fastD)}</td>
      <td>${formatNumber(attach6.classifiedF)}</td>
      <td>${formatNumber(attach6.applicableI)}</td>
    </tr>
  `;
  }).join("");
}
...
function renderDetailSummary(item) {
  ...
  setText("detailFastProgress", formatNumber(attach6.fastD));
  setText("detailClassifiedAmount", formatNumber(attach6.classifiedF));
  setText("detailExcludedAmount", formatNumber(attach6.excludedH));
  setText("detailApplicableAmount", formatNumber(attach6.applicableI));
}
...
function saveItemsOnly() {
  localStorage.setItem("esc_items_only_v4", JSON.stringify(state.items));
  alert("내역서를 저장했습니다.");
}

function loadItemsOnly() {
  const raw = localStorage.getItem("esc_items_only_v4") || localStorage.getItem("esc_items_only_v3");
  if (!raw) {
    alert("저장된 내역서가 없습니다.");
    return;
  }

  state.items = JSON.parse(raw).map(normalizeItem);
  state.selectedItemId = state.items[0]?.id || null;
  renderEstimateWorkspace();
  renderAllReportsOnly();
  alert("내역서를 불러왔습니다.");
...
function normalizeItem(item = {}) {
  ...
  if (normalized.machine == null) {
    normalized.machine = toNumber(normalized.machineDomestic) + toNumber(normalized.machineForeign);
  }
  delete normalized.machineDomestic;
  delete normalized.machineForeign;
  normalized.planProgress = toNumber(normalized.planProgress);
  normalized.actProgress = toNumber(normalized.actProgress);
  normalized.psAmount = toNumber(normalized.psAmount);
  normalized.deduction = toNumber(normalized.deduction);
  return normalized;
}
...
function buildAttachment3Rows(data) {
  ...
  rawRows.forEach(([name, amount, baseIndex, compareIndex]) => {
    const coeff = Math.round((amount / total) * 10000) / 10000;
    const ratio = Math.floor((compareIndex / baseIndex) * 1000000) / 1000000;
    const weighted = Math.floor((coeff * ratio) * 100000000) / 100000000;
    rows.push({ name, amount, coeff, baseIndex, compareIndex, ratio, weighted });
  });
...
function collectData() {
  ...
  data.escRound = toNumber(data.escRound);
  ...
  data.safetyRate = toNumber(data.safetyRate);
  ...
  const attach6Totals = data.filteredItems.reduce((acc, item) => {
    const row = calcAttach6Values(item);
    acc.amountA += row.amountA;
    acc.planB += row.planB;
    acc.actC += row.actC;
    acc.fastD += row.fastD;
    acc.psE += row.psE;
    acc.classifiedF += row.classifiedF;
    acc.deductionG += row.deductionG;
    acc.excludedH += row.excludedH;
    acc.applicableI += row.applicableI;
    return acc;
  }, {
    amountA: 0, planB: 0, actC: 0, fastD: 0, psE: 0,
    classifiedF: 0, deductionG: 0, excludedH: 0, applicableI: 0
  });
  data.attach6Totals = attach6Totals;
...
function renderSummary(data) {
  ...
  daysBadge.textContent = data.periodPass ? "90일 이상 충족" : "90일 미달";
  kBadge.textContent = data.ratePass ? "±3% 이상 충족" : "±3% 미달";
}

function renderCover(data) {
  setText("coverProjectName", `◈ 공사명 : ${data.contractName}`);
  setText("coverRound", `${data.escRound}차 ESC`);
...
function renderSubmitPage(data) {
  setText("submitDemandOrg", data.demandOrg);
  setText("submitContractName", data.contractName);
  setText("submitContractor", data.contractor);
  setText("submitRound", `${data.escRound}차`);
  setText("submitReportDate", formatDate(data.reportDate));
  setText("submitManager", `${data.managerName} ${data.managerTitle}`);
}

function renderTOC() {
  const body = $("tocBody");
  const rows = [
    ["붙임 1", "물가변동으로 인한 계약금액 조정에 대한 종합의견서"],
    ["붙임 2", "물가변동으로 인한 계약금액 조정 검토서"],
    ["붙임 2-1", "기간 및 지수조정율 판정표"],
    ["붙임 3", "물가변동 조정율(지수 조정율) 산출표"],
    ["붙임 4", "물가변동에 적용한 각종지수의 산정표"],
    ["붙임 5", "물가변동 적용대가 산정 내역"],
    ["붙임 6", "물가변동 적용대가 분류 내역서"],
    ["붙임 6-1", "물가변동 적용대가 분류 내역서"],
    ["붙임 6-2", "물가변동 적용대가 산출근거"],
    ["붙임 7", "물가변동 적용대가의 비목군 분류 집계표"],
    ["붙임 8", "물가변동 적용대가의 비목군 분류표"],
    ["붙임 9", "물가변동 적용대가의 비목군 분류 일위대가표"],
    ["붙임 10", "물가변동 적용대가의 비목군 분류 산출근거"]
  ];

  body.innerHTML = rows.map((row, index) => `
    <tr>
      <td style="width: 140px;">${row[0]}</td>
      <td>${row[1]}</td>
...
function renderAttach1(data) {
  ...
  setText("a1ReportDate", formatDate(data.reportDate));
  setText("a1JudgeText", data.finalJudge ? "계약금액 조정 가능" : "계약금액 조정 불가");

  const reasons = [];
  reasons.push(data.periodPass ? "기간 요건(90일 이상) 충족" : "기간 요건(90일 이상) 미충족");
  reasons.push(data.ratePass ? "지수조정율 요건(±3% 이상) 충족" : "지수조정율 요건(±3% 이상) 미충족");
  setText("a1JudgeReason", reasons.join(" / "));
}

function renderAttach2(data) {
  setText("a2ProjectTitle", `◈ 공사명 : ${data.contractName}`);
  setText("a2ContractAmount", formatCurrency(data.contractAmount));
  setText("a2ExcludedAmount", formatCurrency(data.excludedAmount));
  setText("a2ApplicableAmount", formatCurrency(data.applicableAmount));
  setText("a2BidDate", formatDate(data.bidDate));
  setText("a2ContractDate", formatDate(data.contractDate));
  setText("a2AdjustDate", formatDate(data.adjustDate));
  setText("a2PrevAdjustDate", formatDate(data.prevAdjustDate));
  setText("a2ElapsedDays", `${formatNumber(data.elapsedDays)}일`);
  setText("a2PlanProgress", formatPercent(data.plannedProgress, 2));
  setText("a2ActualProgress", formatPercent(data.actualProgress, 2));
  setText("a2FinalK", formatPercent(data.finalKPercent, 4));
  setText("a2FinalAdjustAmount", formatCurrency(data.finalAdjustAmount));
}

function renderAttach21(data) {
  setText("a21ProjectTitle", `◈ 공사명 : ${data.contractName}`);
  setText("a21PeriodJudge", data.periodPass ? "적합" : "부적합");
  setText("a21RateJudge", data.ratePass ? "적합" : "부적합");
  setText("a21FinalJudge", data.finalJudge ? "계약금액 조정 가능" : "계약금액 조정 불가");
}
...
function renderAttach3(data) {
  ...
  body.innerHTML = data.attach3Rows.map((row) => `
    <tr>
      ...
      <td>${row.ratio.toFixed(6)}</td>
      <td>${row.weighted.toFixed(8)}</td>
      <td>-</td>
    </tr>
  `).join("");
  ...
  setText(
    "attach3Foot",
    `지수조정율(K값) = (${data.finalKDecimal.toFixed(4)} - 1) × 100 = ${data.finalKPercent.toFixed(4)} %`
  );
}
...
function renderAttach6(data) {
  ...
  if (!data.filteredItems.length) {
    body.innerHTML = `<tr><td colspan="16">조건에 해당하는 내역이 없습니다.</td></tr>`;
    return;
  }

  const rows = data.filteredItems.map(item => {
    const row = calcAttach6Values(item);
    return `
    <tr>
      <td>${item.code || "-"}</td>
      ...
      <td>${item.spec || "-"}</td>
      <td>${item.unit || "-"}</td>
      <td>${formatCurrency(row.amountA)}</td>
      <td>${formatCurrency(row.planB)}</td>
      <td>${formatCurrency(row.actC)}</td>
      <td>${formatCurrency(row.fastD)}</td>
      <td>${formatCurrency(row.psE)}</td>
      <td>${formatCurrency(row.classifiedF)}</td>
      <td>${formatCurrency(row.deductionG)}</td>
      <td>${formatCurrency(row.excludedH)}</td>
      <td>${formatCurrency(row.applicableI)}</td>
      <td>${item.note || "-"}</td>
    </tr>
  `;
  }).join("");

  const t = data.attach6Totals;
  body.innerHTML = rows + `
    <tr class="total-row">
      <td colspan="6">합계</td>
      <td>${formatCurrency(t.amountA)}</td>
      <td>${formatCurrency(t.planB)}</td>
      <td>${formatCurrency(t.actC)}</td>
      <td>${formatCurrency(t.fastD)}</td>
      <td>${formatCurrency(t.psE)}</td>
      <td>${formatCurrency(t.classifiedF)}</td>
      <td>${formatCurrency(t.deductionG)}</td>
      <td>${formatCurrency(t.excludedH)}</td>
      <td>${formatCurrency(t.applicableI)}</td>
      <td>-</td>
    </tr>
  `;
}
...
function renderAttach9And10(data) {
  ...
  if (!data.filteredItems.length) {
    body9.innerHTML = `<tr><td colspan="22">조건에 해당하는 내역이 없습니다.</td></tr>`;
    body10.innerHTML = `<tr><td colspan="22">조건에 해당하는 내역이 없습니다.</td></tr>`;
    return;
  }
...
function saveDraft() {
  ...
  localStorage.setItem("esc_report_form_v7", JSON.stringify({
    fields: saved,
    items: state.items,
...
function loadDraft() {
  const raw = localStorage.getItem("esc_report_form_v7") || localStorage.getItem("esc_report_form_v6");
  if (!raw) {
    alert("저장된 입력값이 없습니다.");
    return;
  }
